...
(async () => {
    ...
    const b64content = fs.readFileSync(__dirname + "/out.png", { encoding: "base64" })

    twitter.post("media/upload", { media_data: b64content }, function (err, data: any, response) {
        let media_id = data.media_id_string
        twitter.post("account/update_profile_banner", { media_id: media_id }, function (err, data, response) {
            if (err) {
                console.error(err)
            } else {
